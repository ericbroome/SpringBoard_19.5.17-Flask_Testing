...
class Boggle():
    ...
    def make_board(self, width=5):
        """Make and return a random boggle board."""
        self.width = width
        board = []

        for y in range(width):
            row = [choice(string.ascii_uppercase) for i in range(width)]
            board.append(row)
    ...
    def find_from(self, board, word, y, x, seen):
        ...
        if y < self.width and x < self.width:
            if self.find_from(board, word[1:], y + 1, x + 1, seen):
                return True

        if x > 0 and y < self.width:
            if self.find_from(board, word[1:], y + 1, x - 1, seen):
                return True
